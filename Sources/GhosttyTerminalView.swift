...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    private func createSurface(for view: GhosttyNSView) {
        ...
        // Convert env dict into the C array ghostty_surface_new consumes. Without
        // this the shell starts with only ghostty's own env and OSC 7 / CMUX_*
        // injection silently break (regression: 0.63.0 upstream merge).
        GhosttySurfaceEnvironmentVars.append(from: env, into: &envVars, storage: &envStorage)
    ...
    func forceRefresh(reason: String = "unspecified") {
        // PERF: typing hot path (keyDown.textInput); keep diagnostic alloc inside #if DEBUG.
        #if DEBUG
