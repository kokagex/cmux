...
final class GhosttyTerminalStartupEnvironmentTests: XCTestCase {
    func testSurfaceEnvironmentVarsAppendConvertsEnvDictToCArray() {
        var envVars: [ghostty_env_var_s] = []
        var storage: [GhosttySurfaceEnvironmentVars.Storage] = []
        GhosttySurfaceEnvironmentVars.append(
            from: [
        ...
        defer { GhosttySurfaceEnvironmentVars.freeStorage(storage) }

        XCTAssertEqual(envVars.count, 3)
        XCTAssertEqual(storage.count, 3)
        let materialized = Self.materializeEnvVars(envVars)
        XCTAssertEqual(materialized["ZDOTDIR"], "/Applications/cmux.app/Contents/Resources/shell-integration")
        XCTAssertEqual(materialized["CMUX_SHELL_INTEGRATION"], "1")
        XCTAssertEqual(materialized["CMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"], "1")
    }

    func testSurfaceEnvironmentVarsAppendOnEmptyEnvIsNoOp() {
        var envVars: [ghostty_env_var_s] = []
        var storage: [GhosttySurfaceEnvironmentVars.Storage] = []
        GhosttySurfaceEnvironmentVars.append(from: [:], into: &envVars, storage: &storage)
        defer { GhosttySurfaceEnvironmentVars.freeStorage(storage) }

        XCTAssertTrue(envVars.isEmpty)
        XCTAssertTrue(storage.isEmpty)
    }

    func testSurfaceEnvironmentVarsAppendPreservesPreviouslyAddedEntries() {
        var envVars: [ghostty_env_var_s] = []
        var storage: [GhosttySurfaceEnvironmentVars.Storage] = []
        GhosttySurfaceEnvironmentVars.append(from: ["FOO": "1"], into: &envVars, storage: &storage)
        GhosttySurfaceEnvironmentVars.append(from: ["BAR": "2"], into: &envVars, storage: &storage)
        defer { GhosttySurfaceEnvironmentVars.freeStorage(storage) }

        XCTAssertEqual(envVars.count, 2)
        let materialized = Self.materializeEnvVars(envVars)
        XCTAssertEqual(materialized["FOO"], "1")
        XCTAssertEqual(materialized["BAR"], "2")
    }

    private static func materializeEnvVars(_ envVars: [ghostty_env_var_s]) -> [String: String] {
        var result: [String: String] = [:]
        for entry in envVars {
            guard let keyPtr = entry.key, let valuePtr = entry.value else { continue }
            result[String(cString: keyPtr)] = String(cString: valuePtr)
        }
        return result
    }
